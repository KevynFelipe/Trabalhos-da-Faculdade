...
public class Garras_da_nevasca_Final_preDB_Para_Testes {
	...
	public static void main(String[] args) {
		...
		switch (clas) { // CRAÇÃO DE PERSONAGEM
			case 1 -> {
				...
				character[2] = 2000; // HP (Valor alto para evitar problemas de falta de HP durante o teste. Valor original: 30)
				character[3] = 0; // MP
			...
			case 2 -> {
				...
				character[2] = 2000; // HP (Valor alto para evitar problemas de falta de HP durante o teste. Valor original: 25)
				character[3] = 1000; // MP (Valor alto para evitar problemas de falta de MP durante o teste. Valor original: 25)
		...
		if (clas == 0) { //Recuperação de HP e MP pós combate do guerreiro
			...
			character[2] = 2000; // HP (Valor alto para evitar problemas de falta de HP durante o teste. Valor original: 20)
			character[3] = 1000; // MP (Valor alto para evitar problemas de falta de MP durante o teste. Valor original: 25)
		}
		pausar(3000);
		...
		while (persona[2] > 0 && enemy[2] > 0) { // LOOP COMBATE
			...
			if (persona[4] == 1) { // COMBATE MAGO
				if (turnP == 1) {
					...
					switch (escolhaA) {
						case 1 -> {
							...
							pausar(500);
							if (dado >= 0) { // Acerto de ataque físico. Essa chance está alta para testes. Valor original: 10
								int dano0 = 4 + persona[0]; // Cálculo de dano do ataque maximizado para testes. Originalmente com a função aleatorizadora d4().
						...
						case 2 -> {
							...
							if (persona[3] >= 10) {
								...
								System.out.printf("%nVocê tirou %d no D20!%n", dado);
								pausar(500);
								if (dado >= 0) { // Acerto de bola de fogo. Essa magia possui uma chance alta de acerto, diferente dos outros ataques com dado 20. 100% de chance de acerto apenas para teste. Valor original: 10
									int dano01 = 12 + persona[1]; // Cálculo de dano da bola de fogo maximizado para testes. Originalmente com a função aleatorizadora d12().
				...
				if (turnP == 1) {
					...
					System.out.printf("%nVocê tirou %d no D20!%n", dado);
					pausar(500);
					if (dado >= 0) { // Chance de acerto do ataque. Essa chance está alta para testes. Valor original: 10
						int dano02 = 8 + persona[0]; // Cálculo de dano do ataque. Maximizado para testes. Originalmente com a função aleatorizadora d8().
			...
			if (enemy[2] <= 0) {
				...
				if (enemy[3] == 2) { // COMBATE LADRÃO
					...
					int escolhendo = roll.nextInt(1, 3);
					if (escolhendo == 1) {
						int dado = d20();
